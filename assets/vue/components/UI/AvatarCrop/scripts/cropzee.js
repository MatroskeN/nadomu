...
jQuery.fn.extend({
  cropzee: function (options = {
        ...
        modalAnimation: '',
        // cropzee options
        allowedInputs: ['png','jpg','jpeg', 'webp'], // input extensions supported
        ...
        if (!options.allowedInputs) {
            options.allowedInputs = ['png','jpg','jpeg', 'webp'];
        }
        ...
        window.cropzeeCreateImage = function(id) {
            ...
            for (let i = 0; i < cropzeePreviewersLength; i++) {
                ...
                $('.image-previewer, .delAvatar').show();
            }
            ...
            '<div class="light-modal" id="cropzee-modal" role="dialog" aria-labelledby="light-modal-label" aria-hidden="false" data-lightmodal="close">'
                + '<div class="light-modal-content ' + animation + '">'
                    ...
                    + '<div class="light-modal-footer" style="justify-content: space-between;">'
                        + '<div onclick="closeModal()" class="light-modal-close-btn" style="cursor: pointer;" aria-label="close">Закрыть</div>'
                       // + '<div onclick="cropzeeRotateImage(`' + id + '`);" class="light-modal-close-btn" style="cursor: pointer;">Rotate 90deg</div>'
                        + '<div onclick="confirmCrop();" class="light-modal-close-btn" style="cursor: pointer;">Применить</div>'
                    + '</div>'
                + '</div>'
                ...
                reader.onload = function (e) {
                    ...
                    if (cropzeePreviewersLength) {
                        for (let i = 0; i < cropzeePreviewersLength; i++) {
                            ...
                            $('.image-previewer, .delAvatar').hide();
                            ...
                            img.onload = function(){
                                ...
                                //костыль
                                window.original_image_width = this.width;
                                window.original_image_height = this.height;
                                if (this.width < 300 || this.height < 300) {
                                    showErrorModal('Ошибка загрузки аватара', 'Изображение слишком маленькое <BR>' +
                                        'Загрузите изображение не меньше 300 пикселей!');
        ...
        $(this).click(function(){
            ...
            $(this).one("change", function(){
                ...
                if($.inArray(ext, options.allowedInputs) == -1) {
                    alert('Некорректное расширение файла. Допустимые форматы для загрузки: jpg, jpeg, png, webp');
                    resetFileInput(cropzeeInputId);
                } else {
